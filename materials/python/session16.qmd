...
DATA_DIR = Path("../../lab/python/data/citibike/JC")
citibike_path = DATA_DIR / "JC2025.csv"
citibike_zip = DATA_DIR / "JC2025.csv.zip"
weather_path = DATA_DIR / "jersey_weather_2025.csv"
neighborhood_path = DATA_DIR / "jersey-city-neighborhoods.geojson"
```
...
if not citibike_path.exists():
    with ZipFile(citibike_zip, "r") as zip_ref:
...
In case of longer exectuion you can try with the stratified sampling, by selecting `50%` of the data per each customer.

We need stratifed sampling in order to maitain proportion for each month.

Checkout [thuis example.](https://hovhannisyan91.github.io/aca/materials/python/session10.html#stratified-sampling-proportional-to-the-category-distribution)
...
The Core level provides a **Pythonic** way to interact with database schemas and run SQL commands without defining full classes.

While SQLAlchemy is meant to abstract SQL, you can still run raw commands using the `text()` function:
...
In Core, you use the MetaData object and the Table class to define your schem.
...
In ORM, you usually define Python classes, and those classes inherit from Base:
...
After the result we need to fetch the data
...
- **Core** when you want table/query control.
- **ORM + Base** when you want to work with Python classes/objects mapped to database tables.


### Resources

- [Video about SQlAlchemy](https://www.youtube.com/watch?v=529LYDgRTgQ)
