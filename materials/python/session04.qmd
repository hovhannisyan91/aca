...
Now we can drop the `user_id` column as well.


```{python}
df_train_analytics_final.drop(columns='user_id',inplace=True)
```
...
```python
df_train_analytics_final.to_csv("../data/processed/instacart.csv")
```
