...
::: {.callout-note, title = "**When to Use:**"}
...
::: {.callout-tip title = "**Visual Insight:**"}
...
def make_corr_data(r, n=18, noise=0.0):
    """
    Generate 2D points with approximate Pearson correlation r.
    For |r|=1, we create a near-perfect line with tiny jitter.
    """
    if abs(r) == 1.0:
        x = np.linspace(-2, 2, n)
        y = r * x
...
for label, r in targets:
    ax = axes[label]
    x, y = make_corr_data(r, n=16, noise=0.05 if abs(r)==1 else 0.0)
    
    ax.scatter(x, y, s=18)
    ax.set_title("")  # no per-panel title
    
    # minimalist axes like the source image
    ...
    ax.set_ylabel("y", fontsize=9, labelpad=2, rotation=0)
    
    # r annotation and panel letter
    ax.text(0.60, 0.82, f"r = {r:+.1f}".replace("+",""),
            transform=ax.transAxes, fontsize=9)
    ax.text(0.50, -0.18, label,
...
>**Reality:**
>Both are driven by a third factor — hot weather. People buy more ice cream, and people also go swimming more.Swimming increases exposure to drowning risk.
...
ice_df = pd.read_csv('../../data/correlation/IceCream.csv')

fig, ax1 = plt.subplots()
ax1.plot(ice_df["Month"], ice_df["IceCream_Sales"], marker="o")
ax1.set_xlabel("Month")
ax1.set_ylabel("Ice Cream Sales")

ax2 = ax1.twinx()
ax2.plot(ice_df["Month"], ice_df["Drowning_Incidents"], color="tab:red", marker="o")
...
> Deploying more firefighters does **not** increase damage — the causality is reversed.
...
fire_df = pd.read_csv('../../data/correlation/Firefighters.csv')

fig, ax1 = plt.subplots()
ax1.plot(fire_df["Event"], fire_df["Firefighters"], marker="o")
ax1.set_xlabel("Event")
ax1.set_ylabel("Firefighters")

ax2 = ax1.twinx()
ax2.plot(fire_df["Event"], fire_df["Damage_kUSD"], color="tab:red", marker="o")
ax2.set_ylabel("Damage (k USD)")
...
##### References  
`Gelman, Andrew & Jennifer Hill. Data Analysis Using Regression and Multilevel/Hierarchical Models. Cambridge University Press, 2006.`
...
> Coffee does not *cause* higher salaries — job level does.

`The common cause was job seniority`
...
salary_df = pd.read_csv('../../data/correlation/SalaryCoffee.csv')

plt.plot(salary_df["Salary_kUSD"], salary_df["Coffee_Cups"], marker="o")
plt.xlabel("Salary (k USD)")
plt.ylabel("Coffee Cups per Day")
plt.title("Salary vs Coffee Consumption")
plt.show()
```

##### References  

`Davenport, Thomas H., and Jeanne Harris. Competing on Analytics. Harvard Business Press, 2007.`
...
shoe_df = pd.read_csv('../../data/correlation/ShoeSize.csv')

plt.scatter(shoe_df["ShoeSize"], shoe_df["ReadingScore"])
plt.xlabel("Shoe Size")
plt.ylabel("Reading Score")
plt.title("Shoe Size vs Reading Score")
#TODO save as a png file late
# plt.savefig("../../img/correlation/shoesize_reading.png", dpi=300, bbox_inches="tight")
plt.show()
```

##### References  

`Gravetter, Frederick & Larry Wallnau. Statistics for the Behavioral Sciences. Cengage Learning.`
...
`The mechanism was carcinogenic exposure`
