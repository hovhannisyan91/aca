...
- No non-key attribute depends on part of a [composite key](session2.qmd#composite-key)   
...
```sql
INSERT INTO analytics.customer_locations (city_id, geom)
SELECT
  city_id,
  ST_GeomFromText(wkt, 4326)
...
**In analytical databases**, information is intentionally **normalized**.  
This means that meaningful analysis almost always requires **combining multiple tables**.


- Customers are stored separately from transactions  
- Products are stored separately from sales  
- Geography is stored separately from users  

**Joins** are the mechanism that makes analysis possible.

A `JOIN` combines rows from two tables based on a **logical relationship**.


- One table contains a **primary key**  
- Another table contains a **foreign key**  
- Matching values define how rows are aligned  


Conceptually, joins answer the question:

> **Which rows belong together?**


Before writing a `JOIN`, always identify roles.
 

- **Base table**: defines the population you care about  
- **Lookup table**: enriches the base table with attributes  


This decision determines whether rows are preserved or dropped.

**In SQL we are differentiating:**
...
Join results depend on **relationship type**.
...
Many-to-many joins can **multiply rows** and must be handled carefully.
...
**Question:**  
Which orders have valid customers?
...
This **behaves like an INNER JOIN** because of the `WHERE` clause.
...
### One-to-many join: orders → order_items
...
SELECT
  ...
  oi.quantity
FROM analytics.orders AS o
INNER JOIN analytics.order_items AS oi
...
### Multi-table join: customers → orders → order_items → products

**Question:**  
Full sales line items with customer and product info.
...
FROM analytics.customers AS c
INNER JOIN analytics.orders AS o
  ON c.customer_id = o.customer_id
INNER JOIN analytics.order_items AS oi
  ON o.order_id = oi.order_id
...
### JOIN with geography (non-spatial)
...
INNER JOIN analytics.customers AS c
  ON o.customer_id = c.customer_id
INNER JOIN analytics.cities AS ci
  ON c.city_id = ci.city_id
INNER JOIN analytics.regions AS r
  ON ci.region_id = r.region_id
INNER JOIN analytics.countries AS co
...
**Question:**  
Validate that customer points fall inside their city boundaries.
...
SELECT
  ...
  ci.city_name
FROM analytics.customers AS c
INNER JOIN analytics.customer_locations AS cl
  ON c.customer_id = cl.customer_id
INNER JOIN analytics.city_boundaries AS cb
  ON c.city_id = cb.city_id
WHERE ST_Contains(cb.geom, cl.geom);
```
