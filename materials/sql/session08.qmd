---
title: "Session 08: DA with SQL | JOINs"
categories: [SQL, Normalization, JOINs]
image_path: "/img/sql/08"
---
...
[Here](https://github.com/hovhannisyan91/sql-analytics-portfolio/tree/main) you may find the complete solution.

**In case of failure you can use below scripts:**

- **Windows Users:**
  - `reset-db-full.ps1` complete re-run the docker containers deleting the containers 
  - `reset-db.ps1` reruning the postgres db keeping pgadmin info there
- **MacOS Users:**
- `reset-db-full.sh` complete re-run the docker containers deleting the containers 
- `reset-db.sh` reruning the postgres db keeping pgadmin info there

Open the terminal/powershell in `vscode` and typ
...
![]({{< meta image_path >}}/analytics_sceme.png)
...
![]({{< meta image_path >}}/ERD_Analytics_Schema.png)
...
![]({{< meta image_path >}}/checking_schema.png)
...
1. Normalize [this](https://docs.google.com/spreadsheets/d/1ddpzF3RGDkUmIw8-XsLtoi4KM-4acvDa_JBCfDYS3dw/edit?gid=0#gid=0) table
2. Answer to this questions from our database**
...
3. Start thinking abouth [final project.](session13.qmd)