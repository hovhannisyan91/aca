---
title: "Session 02: Intermediate Visual Analytics"
...
::: {.callout-tip title = "Dowloading with Pytho"}
...
Now, we will download the quarterly order files using `Python`. We have `2023_Q1` - `2023_Q5` 
order files.
...
::: {.callout-important title = "how to store"}
...
Befor jumping detailed comparison, let's wathch the [this video](https://www.youtube.com/watch?v=FACIU67GE3I)
...
1. Validate that no duplication appears in aggregated view  
...
![]({{< meta image_path >}}/w2_blending.png)
...
- `orders.csv` ↔ `customers.csv` **ON** `Customer ID = Customer ID`
- `customers.csv` ↔ `customer_addresses.csv` **ON** `Customer ID = Customer ID`
- `orders.csv` ↔ `order_details.csv` **ON** `Order ID = Order ID`
- `order_details.csv` ↔ `products.csv` **ON** `Product ID = Product ID`
- `orders.csv` ↔ `returns.csv` **ON** `Order ID = Order ID`
- `orders.csv` ↔ `salespeople.csv` **ON** `Salesperson ID = Salesperson ID`

::: {.callout-important title = "Important Behavior of Relationships"}
...
::: {.callout-important title = "Box Plot Components"}
...
::: {.callout-tip title = "SQL Comparison"}
...
<!-- TODO: screenshot showing the recent date -->
...
[Tableau Course Code Repository](https://github.com/hovhannisyan91/tableau-analytics-portfolio/tree/main/session02) for this session is available in the GitHub repository linked above. It includes:
