...
- Navigating the Tableau interface (Data Pane, Shelves, Marks Card, Filters)  
...
- Applying Workbook Formatting: Fonts, Colors, and Number Display  
- Task: Build and publish a simple Tableau dashboard using the Airbnb dataset
...
- Advanced charts: area, stacked bar, bullet, bar-in-bar, box plot, dual-axis, histogram, heatmap, highlight table, treemap
...
- Types of filters (extract, data source, context, dimension, measure, date, Top N)
- Task: Build an interactive sales dashboard using calculated fields, LOD expressions, and parameters
  

#### Week 3: Advanced Analytics

- Advanced Table Calculations (running totals, percent of total, rank, window functions, differences, moving averages, nested calculations, conditional table calculations)
- Date Functions (DATEADD, DATEDIFF, DATETRUNC, DATEPART, DATENAME, DATEPARSE, MAKEDATE, MAKEDATETIME, MAKETIME, ISDATE, TODAY, NOW)
- Date Parameters (custom date part selection, dynamic time aggregation, dynamic KPI calculations, multi-source dashboards)
- Cohort Analysis and Retention Analysis in Tableau
- Spatial Analytics (spatial relationships, spatial joins, spatial functions)
- Mapping in Tableau (map layers, map styling, proportional symbol maps, density maps, choropleth maps)
- Cleaning and Reshaping Data in Tableau (NULL handling, data type corrections, removing duplicates, splitting columns, pivoting, hierarchies)
- Using Tableau Prep for Advanced Data Preparation (Tableau Prep vs Tableau Desktop, when to clean outside Tableau)
- Work on Citi Bike dataset (spatial and mapping analysis) and Online Retail dataset (cohort analysis, data cleaning); Olympic Medals dataset (pivoting)
